/* VIEW
    ...
    --                      존재하지 않으면 새로 생성.
    -- 2) FORCE / NOFORCE 옵션 --> 데이터베이스 이관 시 많이 사용
...
-- ORA-01031: 권한이 불충분합니다 
  --> 1) SYS 관리자 계정 접속
  --> 2) ALTER SESSION SET "_ORACLE_SCRIPT" = TRUE; --> 계정명 앞에 붙는 C##을 쓰지 않도록 설정
...
AS SELECT EMP_ID 사번, EMP_NAME 이름, DEPT_TITLE 부서명, JOB_NAME 직급명
	FROM EMPLOYEE
	JOIN JOB USING(JOB_CODE)
	JOIN DEPARTMENT ON(DEPT_CODE = DEPT_ID); -- ORA-00955: 기존의 객체가 이름을 사용하고 있습니다. => 이름 중복

CREATE OR REPLACE VIEW V_EMP /*(사번, 이름, 부서명, 직급명)*/
AS SELECT EMP_ID 사번, EMP_NAME 이름, DEPT_TITLE 부서명, JOB_NAME 직급명
	FROM EMPLOYEE
	JOIN JOB USING(JOB_CODE)
	JOIN DEPARTMENT ON(DEPT_CODE = DEPT_ID); -- OR REPLACE 추가 후 시행 => 성공(기존 V_EMP를 새로운 V_EMP로 덮어쓰기)
...
INSERT INTO V_DCOPY2 VALUES('D0', 'L3'); --  ORA-01400: NULL을 ("KH_KYH"."DEPT_COPY2"."DEPT_TITLE") 안에 삽입할 수 없습니다
...
--> ORA-01400: NULL을 ("KH_KYH"."DEPT_COPY2"."DEPT_TITLE") 안에 삽입할 수 없습니다
*/
...
WITH READ ONLY; -- 읽기 전용 VIEW생성 (SELECT만 가능)


INSERT INTO V_DCOPY2 VALUES('D0', 'L3'); -- ORA-42399: 읽기 전용 뷰에서는 DML 작업을 수행할 수 없습니다.