...
/*  서브쿼리 유형
    ...
    - 다중열 (단일행) 서브쿼리 : 서브쿼리의 SELECT 절에 자열된 항목수가 여러개 일 때
...
SELECT JOB_CODE, AVG(SALARY)	  -- 서브쿼리
FROM EMPLOYEE
GROUP BY JOB_CODE;
...
-- NATURAL JOIN JOB
-- (MAIN.JOB_CODE = JOB.JOB_CODE) 컬럼명 맞지 않아 크로스조인
...
-- 이름 급여 순서로 조회된 결과를 TOP_SAL로 부름
WITH TOP_SAL AS(SELECT EMP_NAME, SALARY FROM EMPLOYEE ORDER BY SALARY DESC)
SELECT *
FROM EMPLOYEE E
JOIN TOP_SAL ON (E.EMP_NAME = TOP_SAL.EMP_NAME)
WHERE ROWNUM <= 10;

-- 직급별 급여 평균보다 급여를 많이 받는 직원의 
-- 이름, 직급코드, 급여 조회
WITH TOP_SAL AS (SELECT JOB_CODE, AVG(SALARY) 평균급여 FROM EMPLOYEE GROUP BY JOB_CODE)
SELECT E.EMP_NAME, E.JOB_CODE, E.SALARY
FROM EMPLOYEE E
JOIN TOP_SAL TS ON (E.SALARY > 평균급여)

WITH TOP_SAL AS (SELECT JOB_CODE, CEIL(AVG(SALARY)) 평균급여 FROM EMPLOYEE GROUP BY JOB_CODE)
SELECT E.EMP_NAME, E.JOB_CODE, E.SALARY, 평균급여
FROM EMPLOYEE E
JOIN TOP_SAL TS ON (TS.JOB_CODE = E.JOB_CODE)
WHERE E.SALARY > 평균급여
