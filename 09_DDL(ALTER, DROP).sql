...
-- ALTER TABLE 테이블명 MODIFY 컬럼명 DEFAULT '값'; -> 데이터 타입 변경

-- ALTER TABLE 테이블명 MODIFY 컬럼명 NULL/NOT NULL; -> NULL 여부 변경

-- 컬럼 삭제
-- ALTER TABLE 테이블명 DROP(삭제할컬럼명);
-- ALTER TABLE 테이블명 DROP COLUMN 삭제할컬럼명; --> 동일한 뜻, 다른 표기법 
--> ＊컬럼 삭제 시 주의사항＊
...
-- LNAME 컬럼 추가 (기본값 '한국')
ALTER TABLE DEPARTMENT_COPY ADD (LNAME VARCHAR2(30) DEFAULT '한국'); --> 컬럼이 생성되면서 DEFAULT 값이 자동 삽입됨.

INSERT INTO DEPARTMENT_COPY 
VALUES ('D10', '개발1팀', 'L1', DEFAULT, DEFAULT);
-- ORA-12899: "KH_KYH"."DEPARTMENT_COPY"."DEPT_ID" 열에 대한 값이 너무 큼(실제: 3, 최대값: 2)
ALTER TABLE DEPARTMENT_COPY MODIFY DEPT_ID VARCHAR(3); -- 2 -> 3 수정
...
SELECT * FROM DEPARTMENT_COPY; -- 확인
COMMIT;

-- 모든 컬럼 삭제
ALTER TABLE DEPARTMENT_COPY DROP(LNAME);
ALTER TABLE DEPARTMENT_COPY DROP(CNAME);
ALTER TABLE DEPARTMENT_COPY DROP(LOCATION_ID);
ALTER TABLE DEPARTMENT_COPY DROP(DEPT_TITLE);
ALTER TABLE DEPARTMENT_COPY DROP(DEPT_ID); -- ORA-12983: 테이블에 모든 열들을 삭제할 수 없습니다

-- 테이블 삭제
DROP TABLE DEPARTMENT_COPY;

-- DEPARTMENT 테이블 복사해서 DEPARTMENT_COPY 테이블 생성
CREATE TABLE DEPARTMENT_COPY
AS (SELECT * FROM DEPARTMENT);

SELECT * FROM DEPARTMENT_COPY; -- 테이블 생성 확인
--> 컬럼명, 데이터타이브, NOT NULL여부만 복사

-- DEPARTMENT_COPY 테이블에 PK 추가(컬럼 : DEPT_ID, 제약조건명 : D_COPY_PK)
ALTER TABLE DEPARTMENT_COPY ADD CONSTRAINT D_COPY_PK PRIMARY KEY (DEPT_ID);
...
ALTER TABLE DEPARTMENT_COPY RENAME CONSTRAINT D_COPY_PK TO DEPT_COPY_PK;


-- 3) 테이블명 변경(DEPARTMENT_COPY -> DCOPY)
ALTER TABLE DEPARTMENT_COPY RENAME TO DCOPY;
SELECT * FROM DEPARTMENT_COPY; -- ORA-00942: 테이블 또는 뷰가 존재하지 않습니다
SELECT * FROM DCOPY; -- 정상실행
...
-- 3) TROP TABLE 삭제옵션 CASCADE CONSTRAINTS 사용	--> CASCADE CONSTRAINTS : 삭제하려는 테이블과 연결된 FK제약 조건을 모두 삭제
...
/* DDL 주의 사항*/
-- 1) DDL (COMMIT/ROLLBACK)이 되지 않는다.
