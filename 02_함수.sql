...
-- 그룹 함수 : N개의 값을 읽어 1개의 결과를 반환(합계, 평균, 최대, 최솟값)
...
-- EMPLOYEE 테이블에서 사번이 짜수인 사원의 사번, 이름 조회
...
SELECT ROUND(123.456, 1) FROM DUAL; -- 123, 두 번째 자리에서 반올림
									--> 두 번째 자리에서 반올림해서
									-- 소수점 한 자리만 표현

SELECT ROUND(123.456, 0) FROM DUAL; -- DEFAULT 0
...
SELECT TRUNC(123.456) 	-- 123 소수점 아래 버림
FROM DUAL;

SELECT TRUNC(123.456, 1) 	-- 123.4 소수점 첫째 자리 아래 버림
FROM DUAL;

SELECT TRUNC(123.456, -1) 	-- 123 10의 자리 아래 버림
FROM DUAL;
...
SELECT LAST_DAY('2022-02-02')  FROM DUAL; -- 윤년 판별
...
SELECT TO_CHAR(1234) FROM DUAL; -- 1,123 -> '1234'
...
SELECT TO_CHAR(1000000, 'L9,999,999') FROM DUAL; -- '\1,000,000' L : LOCAL 그지역의 화폐 단위
...
-- R 패턴 : 1세기를 기준으로 절반(50년) 이상인 경우 이전세기(1900)년대
--                      절반(50년) 미만인 경우 현재세기(2000)년대
...
/* 숫자 형변환 */
-- TO_NUMBER(문자데이터, [포맷]) : 문자형데이터를 숫자 데이터로 변경 (날짜형식은 변환 불가)

SELECT '1,000,000' + 500000 FROM DUAL;
-- ORA-01722: 수치가 부적합합니다

SELECT TO_NUMBER('1000000') + 500000 FROM DUAL;
...
FROM EMPLOYEE
WHERE SALARY = (SELECT MAX(SALARY) FROM EMPLOYEE); -- 서브쿼리
...
SELECT COUNT(DISTINCT DEPT_CODE) FROM EMPLOYEE; -- 7행
...
SELECT SUM(DECODE(SUBSTR(EMP_NO, 8, 1), '1', 1, 0))	-- COUNT로 숫자를 세지 못하는 상황일 때 대안
FROM EMPLOYEE;