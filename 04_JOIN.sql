...
FROM EMPLOYEE
JOIN JOB USING(JOB_CODE); -- ORA-00918: 열의 정의가 애매합니다
...
-- 컬럼 수를 기준으로 JOIN
--> 왼편에 작성된 테이블의 모든 컬럼이 결과에 포함 되어야 한다.
-- ANSI 표준
SELECT EMP_NAME, DEPT_TITLE
FROM EMPLOYEE
...
-- ANSI 표준
SELECT E1.EMP_ID, E1.EMP_NAME, NVL(E1.MANAGER_ID, '없음'), NVL(E2.EMP_NAME, '없음')
FROM EMPLOYEE E1
...
-- DEPT-CODE와 LOCATION_ID가 없는 사원이 있기에 테이블을 JOIN 할 때 LEFT OUTER JOIN
-- BONUS 컬럼이 EMPLOYEE에 있어 예외가 발생 할 수 있음
...
--연봉에 보너스포인트를 적용하시오.
SELECT EMP_NAME, JOB_NAME, SALARY, (SALARY*12 * NVL2(BONUS, BONUS + 1 ,1)) -- (1 + NVL(BONUS, 0))
FROM EMPLOYEE
JOIN JOB USING(JOB_CODE)
