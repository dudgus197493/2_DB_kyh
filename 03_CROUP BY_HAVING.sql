...
FROM EMPLOYEE 
GROUP BY DEPT_CODE; --> DEPT_CODE가 같은 행 끼리 하나의 그룹이 된다.
...
--FROM EMPLOYEE
--GROUP BY 성별 --> SQL 해석 순서에 따라 별칭 SELECT가 GROUP BY보다 늦게 해석되므로 GROUP BY 해석 당시 별칭이 해석이 안 된 상태
-- 별칭은 SELECT에서만 붙일 수 있다.
SELECT DECODE(SUBSTR(EMP_NO, 8, 1), '1', '남', '2', '여'), COUNT(*) "인원 수", SUM(SALARY) "급여 합"
FROM EMPLOYEE
...
-- TO_NUMBER(SUBSTR(HIRE_DATE, 1, 4)) >= 2000
...
-- **그룹은 GROUP BY와 SELECT가 실행될때만 만들어진다.
...
-- EMPLOYEE 테이블에서 직급별 인원수가 5명 이하인 직급코드, 인원수(직급코드 오름차순)
...
- INTERSECT (교집합)	: 두 SELECT 결과 중 중복되느 부분만 조회
