 ...
 * --구조적(조건을 붙여) 질의     언어
 * 
 * 1. DQL 데이터 검색 		SELECT(DML)
 * 2. DML 데이터 조작 		INSERT, UPDATE, DELETE
 * 3. DDL 데이터 정의 		CREATE, DROP, ALTER
 * 4. DCL 데이터 (권한)제어	GRANT, REVOKE
 * 5. TCL 트랜젝션 제어     COMMIT, ROLLBACK
 * 
 * */
...
-- EMPLOYEE 테이블에서 모든 사원의 사번, 이름 급여 * 12(연봉)
...
SELECT EMP_NAME, EMP_ID, HIRE_DATE, SYSDATE
FROM EMPLOYEE;
...
-- 주의사항 : 3) 둘 이상의 컬럼앞에 DISTINCT 튜플 전체 값이 동일한 경우에 한해서 중복을 제거한다.
...
-- WHERE EMP_NAME LIKE '전__'; 예외가 있을 수 있음
...
-- EMPLOYEE 테이블에서 보너스가 없 사원의 이름, 보너스 조회
