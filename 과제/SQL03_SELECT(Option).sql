...
SELECT S.STUDENT_NAME, G.TERM_NO TERM_NAME
FROM TB_STUDENT S
JOIN TB_GRADE G ON (S.STUDENT_NO = G.STUDENT_NO)
...
FROM (
	SELECT S.STUDENT_NO, S.STUDENT_NAME, RANK() OVER(ORDER BY AVG(G.POINT) DESC)
	FROM TB_STUDENT S
	JOIN TB_GRADE G ON (G.STUDENT_NO = S.STUDENT_NO)
	JOIN TB_DEPARTMENT D ON (S.DEPARTMENT_NO = D.DEPARTMENT_NO)
	WHERE D.DEPARTMENT_NAME = '국어국문학과'
...
SELECT STUDENT_NO, STUDENT_NAME
FROM (
	SELECT S.STUDENT_NO, S.STUDENT_NAME, AVG(POINT) 평점
	FROM TB_STUDENT S
	JOIN TB_GRADE G ON (G.STUDENT_NO = S.STUDENT_NO)
...
-- 4. 
--WITH DEPT_KO AS (SELECT DEPARTMENT_NO FROM TB_DEPARTMENT WHERE DEPARTMENT_NAME = '국어국문학과')
--SELECT STUDENT_NO, STUDENT_NAME
--FROM (
--	SELECT S.STUDENT_NO, S.STUDENT_NAME, AVG(POINT) 평점
--	FROM TB_STUDENT S
--	JOIN TB_GRADE G ON (G.STUDENT_NO = S.STUDENT_NO)
--	WHERE S.DEPARTMENT_NO = DEPT_KO.DEPARTMENT_NO
--	GROUP BY S.STUDENT_NO, S.STUDENT_NAME
--	ORDER BY 평점 DESC)
--WHERE ROWNUM <= 1;
...
-- 전공과목 평점을 파악하기 위한 적절한 SQL 문을 찾아내시오.
